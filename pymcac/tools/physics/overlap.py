...
def overlapping(Spheres: xr.Dataset, Aggregates: xr.Dataset) -> xr.Dataset:
    return groupby_aggregate(Spheres, Aggregates,
                             overlap_in_one_agg,
...
def overlap_in_one_agg(data: np.ndarray) -> Tuple[np.float64, ...]:
    ...
    tot_coll = np.float64(0.)
    cij_av = cij_median = Cij_v30 = Cij_v20 = n_c_avg = np.float64(0.)
    ...
    tot_coll = np.float64(2. * k)

    cij, vij = Cij[:k].T

    cij_av: np.float64 = np.mean(cij)
    cij_median: np.float64 = np.median(cij)

    vij_sum: np.float64 = np.sum(vij)
    Cij_v20 = (np.sum(vij * cij ** 2) / vij_sum) ** (1 / 2)
    Cij_v30 = (np.sum(vij * cij ** 3) / vij_sum) ** (1 / 3)

    n_c_avg: np.float64 = np.mean(n_c_i)
