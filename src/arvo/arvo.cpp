...
namespace mcac
{
     ...
     * releases this memory and sets appropriate pointer to NULL.
     ********************************************************************************/
    void Cleanup()
    {
        ind.resize(0);
     ...
     * neighborsNumber) and finally it fills the structure spheres using data from
    ...
    void AvIntegral(const int nArcs, double &pVolume, double &pArea, const double r1, const double z1)
    {
        ...
        { // cycle over all arcs
            ...
            S = sqrt(A * A - B * B - C * C);
            rr = r * r - A;
            if (fabs(fabs(arcs[idx * 3 + 2]) - 2.0 * PI) < EPS_TWO_PI)
            { // full circle arc
                vIone = 2.0 * PI / S;
    ...
    int PointInCircle(const double t, const double s, const int Circle)
    {
        ...
        double d;
        PointInCircle = 0;
        d = sqrt((t - circles[Circle * 4]) * (t - circles[Circle * 4]) + (s - circles[Circle * 4 + 1]) * (s - circles[Circle * 4 + 1]));
    ...
    int CircleInCircle(const int Circ1, const int Circ2)
    {
        ...
        double d;
        CirclesInCircle = 0;
        d = sqrt((circles[Circ1 * 4] + circles[Circ1 * 4 + 2] - circles[Circ2 * 4]) * (circles[Circ1 * 4] + circles[Circ1 * 4 + 2] - circles[Circ2 * 4]) + (circles[Circ1 * 4 + 1] - circles[Circ2 * 4 + 1]) * (circles[Circ1 * 4 + 1] - circles[Circ2 * 4 + 1]));
        ...
        {
            d = sqrt((circles[Circ1 * 4] - circles[Circ2 * 4]) * (circles[Circ1 * 4] - circles[Circ2 * 4 + 0]) + (circles[Circ1 * 4 + 1] - circles[Circ2 * 4 + 1]) * (circles[Circ1 * 4 + 1] - circles[Circ2 * 4 + 1]));
            if (d < circles[Circ2 * 4 + 2])
            {
                CirclesInCircle = (circles[Circ2 * 4 + 3] > 0) ? 1 : 0;
            }
    ...
    int NewArcs(const int Circle, const int NumAtoms)
    {
        ...
        if (!numAngle)
        { // there are no double intersections of idx-th circles with
            // others
            numCond = 0;
            for (idx = 0; idx < NumAtoms; idx++)
            { // if i-th circle is inside of all other
    ...
    void MakeTsCircles(const int NumAtoms)
    {
        ...
        for (idx = 0; idx < NumAtoms; idx++)
        {
            ...
            a = dx * dx + dy * dy + (sphereLocal[2] + r1 - sphereLocal[(idx + 1) * 4 + 2]) * (sphereLocal[2] + r1 - sphereLocal[(idx + 1) * 4 + 2]) - sphereLocal[(idx + 1) * 4 + 3] * sphereLocal[(idx + 1) * 4 + 3];
            b = 8.0 * r1 * r1 * dx;
            c = 8.0 * r1 * r1 * dy;
            d = 4.0 * r1 * r1 * (dx * dx + dy * dy + (sphereLocal[2] - r1 - sphereLocal[(idx + 1) * 4 + 2]) * (sphereLocal[2] - r1 - sphereLocal[(idx + 1) * 4 + 2]) - sphereLocal[(idx + 1) * 4 + 3] * sphereLocal[(idx + 1) * 4 + 3]);
    ...
    void NorthPoleFix(const int numAtoms)
    {
        double d;
        int idx;
        for (idx = 1; idx < numAtoms; idx++)
        { // all except atom 0
            d = fabs(
                sqrt(
                    (sphereLocal[0] - sphereLocal[idx * 4]) * (sphereLocal[0] - sphereLocal[idx * 4]) + (sphereLocal[1] - sphereLocal[idx * 4 + 1]) * (sphereLocal[1] - sphereLocal[idx * 4 + 1]) + (sphereLocal[2] + sphereLocal[3] - sphereLocal[idx * 4 + 2]) * (sphereLocal[2] + sphereLocal[3] - sphereLocal[idx * 4 + 2])) -
                sphereLocal[idx * 4 + 3]);
            if (d < EPS_NORTH_POLE)
            {
    ...
    {
        // nls was originally neighborsNumber[Atom]+1, shifted to neighborsNumber[Atom]!!!
        pVolume = 0;
        pArea = 0;
        ...
        else
        {
            ...
            { // all circles are negative oriented - we have computed complement
